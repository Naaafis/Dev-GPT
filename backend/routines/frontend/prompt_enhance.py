...
class PromptEnhanceRoutine:
    """
    Routine to enhance prompt of the user input
    This routine will need to read frontend text contents and enhance prompt for review.
    """
    def __init__(self, base_config, prompt_read_config, prompt_write_config, prompt_writing_function_map):
        self.base_config = base_config
        self.prompt_read_config = prompt_read_config
        self.prompt_write_config = prompt_write_config
        self.promot_writing_function_map = prompt_writing_function_map
        
        termination_msg = lambda x: isinstance(x, dict) and "TERMINATE" == str(x.get("content", ""))[-9:].upper()

        PROMPTENHANCER_AUTO_REPLY = """
            Ensure that you have fully enhanced every component required for the client's idea. Reflect on the current enhanced prompt.
            Are there any part of prompt that the Dall-e-3 that might need more detailed description or have been overlooked?
            Consider re-evaluating the enhanced prompt for completeness and accuracy. Remember there are a text-to-image model to turn
            this prompt to image. If you are satisfied with the prompt, please reply with 'done'.
        """
        ...
        self.client = UserProxyAgent(
            ...
            default_auto_reply=PROMPTENHANCER_AUTO_REPLY,
